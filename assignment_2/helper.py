...
def attPotentialGrad(q, g, dstargoal, X): #attractive potential gradient
    D = DistancePointToPoint(q, g)
    if D > dstargoal:
        Vx = dstargoal*X*(g[0]-q[0])/D
        Vy = dstargoal*X*(g[1]-q[1])/D
    else:
        Vx = X*(g[0]-q[0])
        Vy = X*(g[1]-q[1])
    return Vx, Vy
...
def repPotentialGrad(q, P, Qstari, n): # repulsive potential gradient
    [D, w, [a, b]] = computeDistancePointToPolygon(q, P)
    if D > Qstari:
        Vx = 0
        Vy = 0
    else:
        Vx = n*((1/Qstari)-(1/D))*(a-q[0])/(D**3)
        Vy = n*((1/Qstari)-(1/D))*(b-q[1])/(D**3)
    return Vx, Vy


def normalisation(q):               #to normalise the point, get unit vector
    s = sqrt(q[0]**2)+(q[1]**2)
    if s == 0:
        Vx = 0
        Vy = 0
